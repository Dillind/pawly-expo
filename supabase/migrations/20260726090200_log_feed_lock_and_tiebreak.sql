...
declare
  ...
  new_log_id uuid;
begin
  -- Serialise concurrent log_feed calls for this pet: without a lock, two
  -- transactions both read the same READ COMMITTED snapshot, both derive
  -- "this rises the satisfied count" from it, and both return `logged` --
  -- neither sees the other's uncommitted insert. Locking on the pet, not
  -- some finer key, matches the granularity the derivation itself is scoped
  -- to (slot_states is a per-pet, per-day computation). Transaction-scoped
  -- so it releases automatically at commit or rollback; taken before the
  -- RLS-gated pet lookup below is deliberate -- it's a cheap in-memory lock,
  -- and letting an arbitrary caller take it on any uuid before we know
  -- whether the pet even exists costs nothing and reveals nothing back to
  -- them.
  ...
  --
  -- Tiebreak matches the discipline private.slot_states already uses in its
  -- greedy loop (slot_at asc, log_at asc, log_id asc): distance alone is not
  -- unique on the live schedule -- from T = 12:30, lunch 12:00 and morning
  -- 13:00 are both 1800 seconds away -- so scheduled_at then schedule_id
  -- make the choice deterministic instead of arbitrary.
  collision as (
