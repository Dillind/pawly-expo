...
// Every pet's feed times, keyed by pet. `useQueries` rather than one bulk call: the per-pet
// query already exists and is already cached by `PetSection`, so this reads the same keys
// instead of adding a second RPC that would go stale on its own schedule.
