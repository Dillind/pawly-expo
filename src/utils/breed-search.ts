...
// Names carry accents nobody types, so both sides are folded first.
...
// Damerau-Levenshtein, stopped once the row's best beats `max`. The bound is
// what keeps this affordable over 900 names on every keystroke.
const editDistance = (a: string, b: string, max: number): number => {
  ...
  for (let i = 1; i <= a.length; i += 1) {
    ...
    for (let j = 1; j <= b.length; j += 1) {
      ...
      // Transposition: "labardor" should still reach "labrador".
...
// Bands, not numbers: every exact match outranks every fuzzy one.
const NAME_PREFIX = 0;
const WORD_PREFIX = 1;
const CONTAINS = 2;
const FUZZY = 3;
...
const scoreOf = (name: string, term: string): number | null => {
  ...
  // Per word as well as whole name, so a typo in "cocker" still reaches
...
// Below `MIN_SEARCH_LENGTH` the full list comes back, so the picker opens on
// every breed rather than one letter's worth of noise.
