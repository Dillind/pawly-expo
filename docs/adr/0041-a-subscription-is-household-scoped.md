...
# 0041 — A subscription is household-scoped, and the entitlement lives on the User
...
- **If household size should ever affect price, the honest lever is a member cap on the free tier** —
  ...
  to add.
- **Belonging to more than one household is itself a paywall candidate.** A professional pet sitter
  is a different buyer from an Owner, and the switcher that makes it possible already exists.
