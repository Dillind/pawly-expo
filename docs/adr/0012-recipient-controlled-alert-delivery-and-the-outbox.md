...
- **Recipients are resolved at send time, not fanned out at queue time.** `alerts` holds one row per _event_, not per recipient, so a preference changed between queue and delivery is respected. It also means the table cannot answer "who was this delivered to" — only "what happened and when was it sent".
