...
| Vector motion   | `lottie-react-native`                                                                            | Installed                            | The welcome screen's looping scene. Native dependency — adding it required a dev client rebuild. The bundled `assets/animations/welcome.json` is **stock art** under the Lottie Simple License and is a placeholder for a commissioned hero; see the `TODO(CRU-030)` in `welcome-art.tsx` and [docs/research/auth-art-and-motion.md](./research/auth-art-and-motion.md) |
...
| Crash reporting | Sentry (`@sentry/react-native`)                                                                  | **Installed**                        | Initialised in `src/lib/sentry.ts` only when `EXPO_PUBLIC_SENTRY_DSN` is set; root `ErrorBoundary` in `src/app/_layout.tsx`; see "Sentry setup" below                                                                                                                                                                                                                   |
...
The code is wired. Nothing reports until these are added by hand. None of the values live in the repo.

- **`EXPO_PUBLIC_SENTRY_DSN`** — an EAS environment variable in `development`, `preview` and `production`, visibility "plain text" or "sensitive". Also add it to `.env` for local runs. Without it Sentry is not initialised and the app runs normally.
- **`SENTRY_AUTH_TOKEN`** — an EAS environment variable, visibility "secret", in `preview` and `production`. The Sentry plugin and `metro.config.js` use it to upload source maps during the build. Without it the stack traces are minified.
- **`SENTRY_ORG`** and **`SENTRY_PROJECT`** — EAS environment variables beside the token. The config plugin reads them because it is given no `organization` or `project`.

The Sentry `environment` tag is `EAS_BUILD_PROFILE`, baked into `extra.sentryEnvironment` at build time, and `development` for a local run. A crash in any screen renders `RootErrorScreen`, which reports to Sentry and offers "Try again".
